...
import { proof, type ProofIssue } from "./proof";
...
export type EvalResult = {
  ...
  facts: {
    ...
    spellingErrors: number; grammarErrors: number; styleFlags: number;
  };
...
function aggregateIssues(issues: ProofIssue[]) {
  const spelling = issues.filter(i => i.type === "spelling");
  const grammar = issues.filter(i => i.type === "grammar");
  ...
  const mapGr: Record<string, GrammarIssue> = {};
  for (const i of [...grammar, ...style, ...other]) {
    const key = `${i.type}:${i.message}`;
    if (!mapGr[key]) mapGr[key] = { type: i.type, message: i.message, example: i.example, count: 0 };
    mapGr[key].count += 1;
  }
  const grammarIssues = Object.values(mapGr).sort((a, b) => b.count - a.count);

  const highlight = issues
  ...
  return {
    ...
    grammarIssues,
    highlight,
    counts: { spelling: spelling.length, grammar: grammar.length, style: style.length, other: other.length }
  };
}
...
export async function evaluateSubmission(
  ...
  const totalErrWeighted = spellingErrors + grammarCore + Math.round(styleFlags * 0.5);
  const errorsPer100 = (totalErrWeighted / Math.max(1, words)) * 100;
  ...
  if (totalErrWeighted > 0) advice.push("Oprav chyby označené žlutě a zvaž návrhy v sekci níže (pravopis, gramatika).");
  ...
  return {
    ...
    facts: {
      ...
      spellingErrors, grammarErrors: grammarCore, styleFlags
    },
