...
export default function WritingTestPage() {
  ...
  const [tokens, setTokens] = useState<{ start: number; end: number }[]>([]); // jen po Evaluate
  ...
  return (
    <div className={`max-w-5xl mx-auto px-4 py-8 grid gap-8 ${text} ${bg} min-h-screen`}>
      ...
      {/* Editor s highlightem AŽ PO EVALUATE */}
      <Card className={`${cardBg} ${cardBorder} ${text}`}>
        <CardContent className="p-6 grid gap-4">
          ...
          <div className={`relative rounded-xl border ${editorBorder}`}>
            <pre
              ...
              aria-hidden="true"
              // žlutý highlight jen z posledního vyhodnocení:
          ...
          {result && (
            <>
              ...
              <div>
                ...
                <ul className={`list-disc pl-5 text-sm ${isDark ? "text-neutral-200" : "text-neutral-700"}`}>
                  {result.coverage.map(c => (<li key={c.id}>{c.covered ? "✅" : "⚠️"} {c.text}</li>))}
                </ul>
              </div>
...
// malý vnitřní komponent na karty se skóre
